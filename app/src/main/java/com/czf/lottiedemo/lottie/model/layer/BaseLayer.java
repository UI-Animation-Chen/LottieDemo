...
import com.czf.lottiedemo.lottie.L;
...
import com.czf.lottiedemo.lottie.animation.content.DrawingContent;
...
import com.czf.lottiedemo.lottie.animation.keyframe.MaskKeyframeAnimation;
import com.czf.lottiedemo.lottie.animation.keyframe.TransformKeyframeAnimation;
...
import com.czf.lottiedemo.lottie.model.content.Mask;
...
public abstract class BaseLayer
    ...
    static BaseLayer forModel(
            CompositionLayer compositionLayer, Layer layerModel, LottieDrawable drawable, LottieComposition composition, Context context) {
        switch (layerModel.getLayerType()) {
            case SHAPE:
                return new ShapeLayer(drawable, layerModel, compositionLayer, composition);
            case PRE_COMP:
                return new CompositionLayer(drawable, layerModel,
                        composition.getPrecomps(layerModel.getRefId()), composition, context);
            case SOLID:
                return new SolidLayer(drawable, layerModel);
            case IMAGE:
                if (isTextType(drawable, layerModel)) {
                    return new Image2TextLayer(drawable, layerModel, context);
                } else {
    ...
    private static boolean isTextType(LottieDrawable drawable, Layer layerModel) {
        if (drawable == null || layerModel == null) {
            return false;
        }
        LottieImageAsset asset = drawable.getLottieImageAssetForId(layerModel.getRefId());
        if (asset != null) {
            return "text:".equals(asset.getDirName());
        }
        return false;
    }
