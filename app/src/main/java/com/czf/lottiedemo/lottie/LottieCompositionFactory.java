...
import com.czf.lottiedemo.lottie.model.LottieCompositionCache;
import com.czf.lottiedemo.lottie.network.NetworkCache;
import com.czf.lottiedemo.lottie.parser.LottieCompositionParser;
...
public class LottieCompositionFactory {
    ...
    public static LottieTask<LottieComposition> fromZipStream(Context context, final ZipInputStream inputStream, @Nullable final String cacheKey) {
        return cache(cacheKey, new Callable<LottieResult<LottieComposition>>() {
            @Override
            public LottieResult<LottieComposition> call() throws Exception {
                return fromZipStreamSync(context, inputStream, cacheKey);
            }
        });
    }
    ...
    private static LottieResult<LottieComposition> fromZipStreamSyncInternal(Context context, ZipInputStream inputStream, @Nullable String cacheKey) {
        ...
        try {
            ...
            while (entry != null) {
                ...
                } else if (entryName.endsWith(".png") || entryName.endsWith(".webp") || entryName.endsWith(".jpg") || entryName.endsWith(".jpeg")) {
                ...
                } else if (entryName.endsWith(".ttf") || entryName.endsWith(".otf")) {
                    // 判断文件名是否含有..
                    if(entryName.contains("../")){
                        inputStream.closeEntry();
                        entry = inputStream.getNextEntry();
