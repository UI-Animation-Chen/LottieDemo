...
import com.czf.lottiedemo.lottie.model.layer.CompositionLayer;
import com.czf.lottiedemo.lottie.model.layer.Image2TextLayer;
...
import com.czf.lottiedemo.lottie.value.SimpleLottieValueCallback;
...
public class LottieAnimationView extends ImageView {
    ...
    private void init() {
        // 为了修复Activity销毁重建时，ClassNotFoundException when unmarshalling异常问题，关闭save能力
        setSaveEnabled(false);
        cacheComposition = true;
        setFallbackResource(0);
        setImageAssetsFolder("");
        setProgressInternal(0, false);
        enableMergePathsForKitKatAndAbove(false, getContext().getApplicationContext());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            BaseLayer hitLayer = findClickedLayer(event);
            if (hitLayer != null) {
                if (mLottieClickListener != null) {
                    mLottieClickListener.onLottieClicked(hitLayer.getName());
                }
            } else {
                if (mLottieClickListener != null) {
                    mLottieClickListener.onLottieClicked("");
                }
            }
        }
        // 不阻拦原始的触摸事件
    ...
    // 找出点击的是哪个layer并返回此layer，返回null表示未找到，表示点击的是整个lottie
    private BaseLayer findClickedLayer(MotionEvent event) {
        ...
        return traverseLayers(compositionLayer, event);
    }

    private BaseLayer traverseLayers(CompositionLayer compositionLayer, MotionEvent event) {
        List<BaseLayer> layers = compositionLayer.getLayers();
        for (BaseLayer layer : layers) {
            if (layer instanceof CompositionLayer) {
                if (!layer.isVisible()) {
                    Log.d("TMe---", "click filterd, comp, not visible: " + layer.getName());
                    continue;
                }
                RectF r = new RectF();
                layer.getBounds(r, null, false);
                if (r.width() < 5 || r.height() < 5) {
                    Log.d("TMe---", "click filterd, comp, rect empty: " + layer.getName() + ", " + r);
                    continue;
                }
                BaseLayer hitLayer = traverseLayers((CompositionLayer) layer, event);
                if (hitLayer != null) {
                    return hitLayer;
                }
            } else {
                if (!layer.isVisible()) {
                    Log.d("TMe---", "click filterd, not visible: " + layer.getName());
                    continue;
                }
                RectF layerTransformedRect = new RectF();
                layer.getBounds(layerTransformedRect, null, false);

                RectF layerOriginRect = new RectF();
                getLayerOriginBounds(layerOriginRect, layerTransformedRect);

                if (isHit(event, layerOriginRect)) {
                    return layer;
                }
            }
        }
    ...
    private boolean isHit(MotionEvent event, RectF rectF) {
        if (event == null || rectF == null) {
            return false;
        }
    ...
    // lottie中layer的bounds是变换之后的，反向获取变换之前的，跟event中x，y可对应的
    private void getLayerOriginBounds(RectF layerOriginRect, RectF layerTransformedRect) {
        // 计算layer映射到屏幕的bound
        ...
        if ((lottieW / lottieH) >= (viewW / viewH)) {
            // 高撑满，宽crop
            scale = viewH / lottieH; // 由lottieH缩放到viewH
            m.preScale(scale, scale);
            m.postTranslate(-((lottieW * scale - viewW) / 2), 0);
        } else {
            // 宽撑满，高crop
            scale = viewW / lottieW; // 由lottieW缩放到viewW
            m.preScale(scale, scale);
     ...
     */
    public void enableMergePathsForKitKatAndAbove(boolean enable, Context context) {
        lottieDrawable.enableMergePathsForKitKatAndAbove(enable, context);
    }
        ...
        if (isInEditMode()) {
            return new LottieTask<>(new Callable<LottieResult<LottieComposition>>() {
                ...
                public LottieResult<LottieComposition> call() throws Exception {
                    return cacheComposition ? LottieCompositionFactory.fromRawResSync(getContext(), rawRes)
    ...
    private LottieTask<LottieComposition> fromAssets(final String assetName) {
        if (isInEditMode()) {
            return new LottieTask<>(new Callable<LottieResult<LottieComposition>>() {
                @Override
                public LottieResult<LottieComposition> call() throws Exception {
                    return cacheComposition ? LottieCompositionFactory.fromAssetSync(getContext(), assetName) : LottieCompositionFactory.fromAssetSync(getContext(), assetName, null);
                }
            }, true);
        } else {
            return cacheComposition ?
    ...
    private Image2TextLayer traverseLayers(CompositionLayer compositionLayer, String id) {
        List<BaseLayer> layers = compositionLayer.getLayers();
        for (BaseLayer layer : layers) {
            if (layer instanceof CompositionLayer) {
                Image2TextLayer hitLayer = traverseLayers((CompositionLayer) layer, id);
                if (hitLayer != null) {
                    return hitLayer;
                }
            } else {
                if (TextUtils.equals(id, layer.getName()) && layer instanceof Image2TextLayer) {
                    return (Image2TextLayer) layer;
                }
            }
        }
        return null;
    ...
    private Image2TextLayer findTimerLayer(String id) {
        if (lottieDrawable == null) {
            return null;
        }
        ...
        return traverseLayers(compositionLayer, id);
    }

    private void prepareTimerIfNeed() {
        if (this.composition != null && lottieDrawable != null) {
            TextDelegate textDelegate = lottieDrawable.getTextDelegate();
            LottieComposition.Timer timer = this.composition.getTimer();
            if (timer != null && textDelegate != null) {
                int startFrame = timer.ke;
                int startFrameAfterTimer = -1;
                int endFrameAfterTimer = -1;
                if (timer.inel != null && timer.inel.length >= 2) {
                    startFrameAfterTimer = timer.inel[0];
                    endFrameAfterTimer = timer.inel[1];
                }
                if (startFrame < 0 || startFrameAfterTimer < 0 || endFrameAfterTimer < 0) {
                    return;
                }

                int timerStartS = -1;
                int timerLenS = -1;
                String s1 = textDelegate.getText(timer.tl);
                String s2 = textDelegate.getText(timer.at);
                try {
                    timerStartS = Integer.parseInt(s1);
                    timerLenS = Integer.parseInt(s2);
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
                if (timerStartS < 0 || timerLenS < 0) {
                    return;
                }

                if (!TextUtils.isEmpty(timer.id)) {
                    Image2TextLayer target = findTimerLayer(timer.id);
                    if (target != null) {
                        this.timerEl = timer.el;
                        this.image2TextLayerTimer = target;
                        this.timerText = timerStartS;
                        this.endTimer = timerStartS - timerLenS;
                        this.startFrameAfterTimer = startFrameAfterTimer;
                        this.endFrameAfterTimer = endFrameAfterTimer;
    ...
    private Image2TextLayer image2TextLayerTimer; // 倒计时图层
    private int timerText; // 倒计时开始的时间点
    private int endTimer; // 倒计时截止时的时间点
    private int startFrameAfterTimer; // 倒计时结束后，从哪帧开始播放
    private int endFrameAfterTimer; // 倒计时结束后，播放到哪帧停止
    private String timerEl; // 倒计时到了的对外事件
    ...
    private final Runnable timerRunnable = new Runnable() {
        @Override
        public void run() {
            if (timerText > endTimer) {
                timerText--;
                image2TextLayerTimer.updateText("" + timerText);
                invalidate();
                startTimer();
            } else {
                playAnimation();
                setFrame(startFrameAfterTimer);
                if (!TextUtils.isEmpty(timerEl) && mLottieClickListener != null) {
                    mLottieClickListener.onLottieClicked(timerEl);
                }
                addAnimatorUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
                        if (getFrame() >= endFrameAfterTimer) {
                            removeUpdateListener(this);
                            pauseAnimation();
                        }
                    }
    ...
    public void setComposition(@NonNull LottieComposition composition) {
        ...
        boolean isNewComposition = lottieDrawable.setComposition(composition, getContext().getApplicationContext());
    ...
    private LottieClickListener mLottieClickListener;

    public void setLottieClicklistener(LottieClickListener l) {
        mLottieClickListener = l;
    }

    public interface LottieClickListener {
        // lottie点击事件，elName为lottieJson中配置的el
        void onLottieClicked(String elName);
    }
}
