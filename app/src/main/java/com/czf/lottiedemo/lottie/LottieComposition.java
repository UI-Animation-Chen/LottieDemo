...
public class LottieComposition {
    ...
    private SparseArray<FontCharacter> characters;
    ...
    public void init(Rect bounds, float startFrame, float endFrame, float frameRate,
            ...
            List<Layer>> precomps, Map<String, LottieImageAsset> images,
                     SparseArray<FontCharacter> characters, Map<String, Font> fonts,
                     List<Marker> markers, Timer timer) {
        this.bounds = bounds;
    ...
    public SparseArray<FontCharacter> getCharacters() {
        return characters;
    }
    ...
    public static class Timer {
        public int ke; //哪一帧开始倒计时
        public String id; //倒计时显示到哪个id的layer上，对应layer的nm属性
        public String tl; //倒计时开始的时间，比如20秒。本身是el表达式，解析完后是秒
        public String at; //倒计时多长时间s，比如8秒，那么从20秒倒计时到12秒触发事件。本身是el表达式，解析后是秒
        public int[] inel; //数组类型，表示继续播放的帧数，第一个元素表示开始播放帧，第二个元素表示结束播放帧。例如[200,250]
        public String el; //对外事件，表示倒计时结束的时候应该触发什么样的事件，字符串传到外界
    }
}